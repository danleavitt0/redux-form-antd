...
import { RadioField as Radio, SelectField as Select } from "./components/MultiSelect";
...
const sliderMap = customMap(({ input: { onChange, value = 0 }, range, min = 0, max = 100 }) => {
...
const selectFieldMap = customMap(({ input: { onChange, value }, multiple, options }) => {
...
const bluredFieldMap = ({ meta: { touched, error, warning, valid } = {}, input: { value, onChange }, ...props }) => ({
  ...props,
  defaultValue: value,
  onBlur: e => {
...
export const CheckboxGroupField = createComponent(CheckboxGroup, checkboxGroupMap);

// will trigger on change only onBlur
// usefull for performance reasons
export const LazyTextField = createComponent(Input, bluredFieldMap);
